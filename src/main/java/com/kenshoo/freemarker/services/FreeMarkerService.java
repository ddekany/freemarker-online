...
public class FreeMarkerService {
    ...
    public FreeMarkerServiceResponse calculateFreeMarkerTemplate(String templateText, Map<String, String> params) {
        ...
        try {
            ...
            logger.info(ERROR_IN_TEMPLATE_PARSING);
            logger.debug(ERROR_IN_TEMPLATE_PARSING, e);
            return new FreeMarkerServiceResponse("", false, e.getMessage());
        }
        try {
            template.process(params, writer);
        } catch (TemplateException e) {
            logger.info(ERROR_IN_TEMPLATE_EVALUATION);
            logger.debug(ERROR_IN_TEMPLATE_EVALUATION, e);
            return new FreeMarkerServiceResponse("", false, e.getMessage());
